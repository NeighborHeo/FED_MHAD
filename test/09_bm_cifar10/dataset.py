...
class Cifar10Partition: 
    def __init__(self, args: argparse.Namespace):
        ...
        # self.testset = torch.utils.data.Subset(torchvision.datasets.CIFAR10(root='~/.data', train=False, download=False, transform=transform_train), range(0, 1000))
    ...
    def init_partition(self):
        self.split_data = get_dirichlet_split_data(self.trainset.data, self.trainset.targets, self.N_parties, self.N_class, self.alpha)
    ...
    def load_partition(self, i: int, alpha=0.1):
        ...
        # 데이터셋 생성
        train_partition = torch.utils.data.Subset(self.trainset, self.split_data[i]["idx"])
